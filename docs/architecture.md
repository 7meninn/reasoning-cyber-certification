...
The Phase 4 app is a local multi-agent workflow with three execution modes:
...
8. Streamlit renders the selected route and exposes `RunTrace` with raw JSON, parsed output, citations, guardrail verdicts, model metadata, retrieval metadata, repair/fallback metadata, route, fallback mode, and latency.
...
Phase 4 still does not use direct raw index search, hosted Agent Framework, Foundry evaluations, or production observability services.
