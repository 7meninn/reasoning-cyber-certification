...
The Phase 5 app is a local multi-agent workflow with three execution modes:
...
10. Streamlit renders the selected route and exposes `RunTrace` with raw JSON, parsed output, lab score, adaptive remediation reason, citations, guardrail verdicts, model metadata, retrieval metadata, repair/fallback metadata, route, fallback mode, and latency.
...
Phase 5 still does not use direct raw index search, hosted Agent Framework, Foundry evaluations, or production observability services.
